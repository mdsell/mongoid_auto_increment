# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-
# stub: mongoid-auto-increment 0.1.3 ruby lib

Gem::Specification.new do |s|
  s.name = "mongoid-auto-increment"
  s.version = "0.1.3"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["Peter Savichev (proton)", "Ross Paffett"]
  s.date = "2014-10-20"
  s.description = "Add SQL like auto-incrementing fields to your Mongoid documents."
  s.email = ["psavichev@gmail.com", "ross@rosspaffett.com"]
  s.extra_rdoc_files = [
    "LICENSE",
    "README.rdoc"
  ]
  s.files = [
    ".rspec",
    ".travis.yml",
    "Gemfile",
    "LICENSE",
    "README.rdoc",
    "Rakefile",
    "VERSION",
    "lib/mongoid_auto_increment.rb",
    "lib/mongoid_auto_increment/incrementor.rb",
    "spec/models/book.rb",
    "spec/models/comment.rb",
    "spec/models/invoice.rb",
    "spec/models/order.rb",
    "spec/models/post.rb",
    "spec/mongoid_auto_increment_spec.rb",
    "spec/spec_helper.rb"
  ]
  s.homepage = "http://github.com/raws/mongoid-auto-increment"
  s.licenses = ["MIT"]
  s.rubygems_version = "2.2.2"
  s.summary = "Auto-incrementing fields for Mongoid documents"

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<mongoid>, ["~> 4.0"])
      s.add_development_dependency(%q<bundler>, [">= 0"])
      s.add_development_dependency(%q<jeweler>, ["~> 1.8.6"])
      s.add_development_dependency(%q<rake>, [">= 0"])
      s.add_development_dependency(%q<rdoc>, [">= 0"])
      s.add_development_dependency(%q<rspec>, [">= 2.0.0"])
      s.add_development_dependency(%q<simplecov>, [">= 0.4.0"])
      s.add_development_dependency(%q<database_cleaner>, [">= 0.8.0"])
      s.add_development_dependency(%q<mongoid-tree>, [">= 2.0.0"])
    else
      s.add_dependency(%q<mongoid>, ["~> 4.0"])
      s.add_dependency(%q<bundler>, [">= 0"])
      s.add_dependency(%q<jeweler>, ["~> 1.8.6"])
      s.add_dependency(%q<rake>, [">= 0"])
      s.add_dependency(%q<rdoc>, [">= 0"])
      s.add_dependency(%q<rspec>, [">= 2.0.0"])
      s.add_dependency(%q<simplecov>, [">= 0.4.0"])
      s.add_dependency(%q<database_cleaner>, [">= 0.8.0"])
      s.add_dependency(%q<mongoid-tree>, [">= 2.0.0"])
    end
  else
    s.add_dependency(%q<mongoid>, ["~> 4.0"])
    s.add_dependency(%q<bundler>, [">= 0"])
    s.add_dependency(%q<jeweler>, ["~> 1.8.6"])
    s.add_dependency(%q<rake>, [">= 0"])
    s.add_dependency(%q<rdoc>, [">= 0"])
    s.add_dependency(%q<rspec>, [">= 2.0.0"])
    s.add_dependency(%q<simplecov>, [">= 0.4.0"])
    s.add_dependency(%q<database_cleaner>, [">= 0.8.0"])
    s.add_dependency(%q<mongoid-tree>, [">= 2.0.0"])
  end
end

